...
function HeroContent({ messageCount }: { messageCount: number }) {
    ...
    <>
      ...
      <div className="relative mx-auto flex max-w-3xl flex-col items-center gap-5 sm:gap-7">
        <div className="relative">
          <div className="hero-glow absolute inset-0 scale-125 rounded-full bg-sky-300/30 blur-3xl" />
          <div className="relative rounded-full border border-white/70 bg-white/90 p-2.5 shadow-lg sm:bg-white/70 sm:p-3">
            <BabyIcon className="h-24 w-24 sm:h-32 sm:w-32" />
          </div>
        </div>

        <div className="relative w-full space-y-4 sm:space-y-5">
          <div className="relative mx-auto max-w-2xl overflow-visible rounded-2xl border border-sky-100/80 bg-white/95 px-5 py-5 shadow-sm sm:rounded-[1.75rem] sm:bg-gradient-to-br sm:from-white/80 sm:via-sky-50/50 sm:to-violet-50/40 sm:px-10 sm:py-8 sm:shadow-[inset_0_1px_0_rgba(255,255,255,0.9)] sm:backdrop-blur-sm">
            <div className="pointer-events-none absolute -inset-px hidden rounded-2xl bg-gradient-to-br from-sky-200/40 via-transparent to-violet-200/40 sm:block sm:rounded-[1.75rem]" />
        ...
        <div className="w-full max-w-2xl rounded-2xl border border-amber-200/70 bg-amber-50/95 px-4 py-5 sm:bg-gradient-to-br sm:from-amber-50/90 sm:to-orange-50/60 sm:px-6 sm:py-6 sm:shadow-inner">
          <p className="font-arabic text-base leading-[2.2] text-amber-950 sm:text-lg sm:leading-[2.3]">
            {siteContent.birthAnnouncement}
          </p>
        </div>

        <blockquote className="relative w-full max-w-2xl rounded-2xl border border-emerald-200/70 bg-emerald-50/95 px-4 py-5 sm:bg-gradient-to-br sm:from-emerald-50/90 sm:to-sky-50/80 sm:px-6 sm:py-7 sm:shadow-inner">
        ...
        {messageCount > 0 && (
          <div className="inline-flex flex-row items-center gap-2 rounded-full border border-rose-200 bg-rose-50/90 px-5 py-2.5 text-sm font-semibold text-rose-700 shadow-sm">
...
export function Hero({ messageCount }: { messageCount: number }) {
  return (
    <section className="celebration-card page-section relative overflow-hidden rounded-3xl border border-white/60 bg-white/92 px-4 py-8 text-center shadow-lg sm:rounded-[2rem] sm:bg-white/55 sm:px-10 sm:py-16 sm:shadow-[0_20px_80px_rgba(126,200,227,0.25)] sm:backdrop-blur-2xl">
      <div className="md:hidden">
        <HeroContent messageCount={messageCount} />
      </div>
